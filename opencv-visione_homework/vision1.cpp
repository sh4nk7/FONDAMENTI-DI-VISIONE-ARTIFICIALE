...
bool ParseInputs(ArgumentList& args, int argc, char **argv) {
  ...
  // Numero di spicchi presente nell'immagine (default: 9)
  args.numspicchi = 9;
...
void drawEdge(const cv::Mat & image, const std::vector<int> & edges_num, cv::Mat & edges, int numspicchi)
{
  ...
  for (int idx : edges_num)
  {
    ...
    // Partenza dall'alto (90°) e senso orario 
    double angle = 90.0 - idx * angleStep;
    double rad = angle * CV_PI / 180.0;

    // Calcolo del punto finale del raggio
    cv::Point p(center.x + std::cos(rad) * image.cols / 2,
                center.y - std::sin(rad) * image.rows / 2);

    // Disegno linea bianca
    cv::line(edges, center, p, cv::Scalar(255), 1, cv::LINE_AA);
  }
}
...
int main(int argc, char **argv)
{
  ...
  std::cout << "File " << args.image << " " << image.rows << "x" << image.cols
